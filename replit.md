...
- **OAuth = e-mail verified automaticamente (Task #205)**: `findOrCreateOAuthUser` chama `markEmailVerifiedFromTrustedProvider(email)` no início — espelha o trust do provider Google/Facebook gravando uma linha `verified=TRUE` em `email_verification_codes` (idempotente via NOT EXISTS, não dá pra usar ON CONFLICT porque a tabela não tem UNIQUE em email). Backfill one-shot roda no final de `initializeSchema()` via `backfillOAuthVerifiedEmails()` pra usuários antigos. `isUserEmailVerified` continua sendo a fonte da verdade — só corrigimos o input. Endpoint `POST /api/auth/resend-verification` (requireAuth) é a contraparte pro fluxo inline (`<EmailVerificationInline/>` em `src/components/`): não exige email no body (pega da sessão), respeita o cooldown de 60s do `RESEND_COOLDOWN_MS`. CreateCommunityModal usa o componente quando POST devolve 403 EMAIL_NOT_VERIFIED — sem perder os campos do form, retoma o submit ao confirmar. Reusar o componente em outros gates de e-mail no futuro.
...
### Comunidade — CMS, criação por usuários e moderação local (Task #198)
...
- Frontend:
  ...
  - `CreateCommunityModal` novo: nome (slug derivado no server), descrição, ícone (emoji picker), regras opcionais.
