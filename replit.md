...
- Status-aware visibility (Task #25 / Task #26): `GET /api/home-feed` LEFT JOINs `content_items` and hides any card whose linked content is not `published`; cards with `content_item_id IS NULL` (static promos) are always visible. The admin list LEFT JOINs the same table and exposes `linked_content_status` + a derived `link_state` (`ok` | `draft` | `archived` | `missing`) so `AdminHomeFeedPage` flags problem cards with a red "conteúdo em rascunho" / "conteúdo removido" badge or a slate "conteúdo arquivado" badge. Integration script: `scripts/test-home-feed-status.ts` covers publish/unpublish/archive/delete transitions.
- Archived state (Task #26): `content_items.status` accepts `draft | published | archived`. Archived rows are excluded from every public listing/detail (same as drafts) but are surfaced distinctly in `AdminCmsPage` (separate filter chip + slate badge + archive/restore button). Endpoints: `POST /api/admin/cms/:id/archive` and `POST /api/admin/cms/:id/unarchive` (the latter restores to `draft`; the producer must explicitly publish again).
